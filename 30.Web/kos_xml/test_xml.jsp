...
<%
	...
	try {
		...
		String strUrl = "http://localhost:8088/kosmoMember/kos_xml/test_xml.xml";
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				alert("	jQuery 함수 들어갔음!");
				
				var xmlText, xmlParser, xmlDoc;
				
				xmlText = '<%= strHtml %>';
				
				alert("	xmlText 는 ?	>>> : " + xmlText);
				console.log("	xmlText 는 ?	>>> : " + xmlText);
				
				xmlParser = new DOMParser();
				xmlDoc = xmlParser.parseFromString(xmlText, "text/xml");
				
				alert("	xmlDoc 는 ?	>>> : " + xmlDoc);
				console.log("	xmlDoc 는 ?	>>> : " + xmlDoc);
				
				$("#parseText").click(function(){
					
					alert("	parseText 클릭해슴! " + parseText);
					console.log("	parseText 클릭을 해슴! " + parseText);
				...
				$("#parseFind").click(function(){
					...
					alert("	parseFind 를 클릭해슴! " + parseFind);
					console.log("	parseFind 를 클릭해슴! " + parseFind);
					
					// find 함수나 each 함수를 사용한다.
	...
	<body>
		...
		<button id = "parseFind">find() 함수로 파싱해보기</button>
		<p id = "text"></p>
	</body>
</html>
